...
<div class="container">
	...
	<h4><b>주문상품</b></h4>
	<div class="border rounded p-3 mb-2 bg-white text-dark shadow-sm orderListItem" data-goodsNo="${vo.goodsNo }" data-categoryNo="${vo.categoryNo }"> <!-- 주문 상품의 정보를 보여주는 div -->
		<!-- 주문 상태 -->
		...
		<div class="text-center">
			...
			<c:if test="${vo.orderState == '주문 확인' }">			
				<button class="btn btn-outline-secondary ml-1 w-25 orderCancleBtn" data-toggle="modal" data-target="#cancleModal">취소 요청</button>
			</c:if>
			<c:if test="${vo.orderState != '취소 요청' && vo.orderState != '구매 확인' && vo.orderState != '구매 확인(리뷰작성)'}">			
				<button class="btn btn-outline-secondary ml-1 w-25 orderConfirmBtn">구매 확인</button>
			</c:if>
			<!-- 리뷰 버튼 -->
	...
	<div class="border rounded p-3 mb-2 bg-white text-dark shadow-sm"> <!-- 결제 정보를 보여주는 div -->
		...
		<div class="row">
			<div class="col-8"> <!-- 글부분(설명) -->
				...
				<div style="border-left: 5px solid #aaa;"> <!-- 앞에 수직으로 선을 만들기 위해 div의 style 사용 -->
					<!-- 카드 정보 출력 -->
					<div class="ml-2 mt-1">${vo.payDetail }</div> <!-- 왼쪽의 마진을 주기위해 span 태그안에 데이터를 넣은 후 ml 클래스로 마진을 준다. -->										
				</div>
			</div>
			        ...
			        <div class="modal-body">
						...
						<div class="form-group">
	 						...
	 						<!-- 결제창에서만 사용하는 데이터이므로 name x -->
						  	<input type="text" class="form-control" id="cancleReason" name="cancleReason" placeholder="취소하는 이유를 입력해주세요.">
						</div>		          	
			        </div>	        
